...
# Note: If you already have a key vault, you could change the values
# below to match your currently deployed environment and then use
# terraform import to bring that key vault into state.
...
resource "azurerm_key_vault_access_policy" "iac_keyvault_atlas_policy" {
    ...
    tenant_id = data.azurerm_client_config.current.tenant_id
    object_id = var.azure_keyvault_accesspolicy_principal_objid
    #application_id = var.azure_keyvault_accesspolicy_principal_appid
...
resource "azurerm_key_vault_access_policy" "iac_keyvault_user_policy" {
    key_vault_id = azurerm_key_vault.iac_keyvault.id
    tenant_id = data.azurerm_client_config.current.tenant_id
    object_id = var.azure_keyvault_accesspolicy_principal_user_objid
    #application_id = var.azure_keyvault_accesspolicy_principal_appid
...
resource "azurerm_key_vault_secret" "iac_atlas_secret" {
  name         = "secret-sauce"
  value        = "sweetandsour"
  key_vault_id = azurerm_key_vault.iac_keyvault.id
